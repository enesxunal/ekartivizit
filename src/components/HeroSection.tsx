...
export default function HeroSection() {
  ...
  return (
    <section className="border-b border-[#e7eae4] bg-white">
      <div className="site-container grid gap-10 py-10 sm:py-14 lg:grid-cols-[.9fr_1.1fr] lg:items-center lg:py-16">
        <div className="max-w-2xl">
          ...
          <h1 className="mt-6 text-[clamp(3.3rem,7vw,6.8rem)] font-semibold leading-[.86] tracking-[-.075em] text-[#171a16]">Baskı ihtiyacın ne?</h1>
          <p className="mt-6 max-w-xl text-base leading-7 text-[#666d63] sm:text-lg">Kartvizitten broşüre, etiketten promosyon ürünlerine kadar baskı ürünlerini seçin, özelliklerini belirleyin ve online sipariş edin.</p>
          ...
          <div className="mt-8 flex flex-wrap gap-x-5 gap-y-2 text-xs text-[#6f756c]">{['KDV dahil fiyatlar','Dosya yükleme','Sipariş takibi'].map((item) => <span key={item} className="inline-flex items-center gap-2"><Check className="size-3.5 text-[#579d32]" />{item}</span>)}</div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 rounded-[24px] bg-[#f3f4f1] p-3 sm:gap-4 sm:p-4">
          {products.map((product, index) => (
            <Link key={product.id} href={product.href} className={`group relative overflow-hidden rounded-[16px] border border-[#e3e6e0] bg-white ${index === 0 ? 'sm:row-span-2' : ''}`}>
              <div className={`relative ${index === 0 ? 'h-full min-h-[320px]' : 'aspect-[4/3]'}`}><Image src={product.image} alt={product.name} fill sizes="(max-width:1024px) 50vw, 28vw" className="object-contain p-5 transition duration-300 group-hover:scale-[1.03]" /></div>
